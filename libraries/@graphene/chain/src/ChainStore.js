...
class ChainStore
{
   ...
   onUpdate( updated_objects ) /// map from account id to objects
   {
      for( let a = 0; a < updated_objects.length; ++a )
      {
         for( let i = 0; i < updated_objects[a].length; ++i )
         {
            ...
            if( validation.is_object_id( obj ) ) { /// the object was removed
   ...
   getAsset( id_or_symbol )
   {
      ...
      if( validation.is_object_id(asset_id)) {
      ...
      if( validation.is_account_name(id_or_account, true) || (id_or_account.substring(0,4) == "1.2."))
      {
         ...
         if( !account )
         {
            this.lookupAccountByName( id_or_account ).then ( account => {
               ...
               let account_id = account.get('id')
               let witness_id = this.witness_by_account_id.get( account_id )
               if( validation.is_object_id( witness_id ) )
      ...
      if( validation.is_account_name(id_or_account, true) || (id_or_account.substring(0,4) == "1.2."))
      {
         ...
         if( !account )
         {
            this.lookupAccountByName( id_or_account ).then( account=>{
               ...
               if( validation.is_object_id( committee_id ) ) return this.getObject( committee_id, on_update )
   ...
   fetchFullAccount( name_or_id )
   {
      ...
      else
      {
         ...
         let account_id = this.accounts_by_name.get( name_or_id )
         if( validation.is_object_id( account_id ) )
   ...
   fetchRecentHistory( account, limit = 100 )
   {
      ...
      /// in flight...
        ...
        if( !validation.is_object_id(account_id) && account.toJS )
